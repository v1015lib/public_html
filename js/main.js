...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('sidemenu').addEventListener('click', (event) => {
        ...
        if (target.matches('.department-link')) {
            ...
            let params = { page: 1 }; // Siempre reseteamos a la página 1

            if (departmentId !== 'all') {
                // Si es un departamento específico, lo establecemos.
                params.department_id = departmentId;
            } else {
                // Si es "Ver todos", explícitamente eliminamos el filtro.
                // Al pasar `null`, le decimos al cargador que ignore este parámetro.
